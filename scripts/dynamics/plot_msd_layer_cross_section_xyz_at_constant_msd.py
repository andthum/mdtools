...
r"""
Plot the diffusion time at which the x-, y- and z-component of the MSD
reach a certain value as function of the initial particle position.
...
-o          Output filename.
--msd       MSD in data units for which to plot the diffusion times as
            function of the initial particle position.  Default: ``1``.
--msd-eq-bins
            Instead of choosing the MSD manually with \--msd, set the
            MSD for each individual bin to the square of the respective
            bin width.  Must not be used together with \--msd.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Plot the diffusion time at which the x-, y- and z-component of"
            " the MSD reach a certain value as function of the initial"
            " particle position.  This script is designed to plot cross"
            " sections from the output of msd_layer_serial.py"
            " (or msd_layer_parallel.py).  For more information, refer to the"
    ...
    group.add_argument(
        ...
        default=1,
        help=(
            "MSD in data units for which to plot the diffusion times as"
    ...
    group.add_argument(
        ...
        action="store_true",
        help=(
            "Instead of choosing the MSD manually with --msd, set the MSD for"
            " each individual bin to the square of the respective bin width."
            "  Must not be used together with --msd."
        ),
    )
    parser.add_argument(
        "-d",
    ...
    msd, ix = mdt.nph.find_nearest(msd, args.MSD, axis=1, return_index=True)
    ...
    with PdfPages(args.OUTFILE) as pdf:
        # Plot diffusion time as function of initial position
        ...
        for i in range(len(args.MDFILES)):
            ax1.plot(bin_centers, times[i], label=labels[i], marker=markers[i])
        if args.LOGY:
        ...
        if args.MDFILES is not None and not args.MSD_EQ_BINS:
            ...
            legend_title = r"Var$[\Delta r(\Delta t)] =$ (Bin Width)$^2$"
        elif args.MDFILES is None and not args.MSD_EQ_BINS:
            legend_title = (
                r"$\langle \Delta r^2(\Delta t) \rangle ="
                + str(args.MSD)
                + r"$ "
                + args.LUNIT
                + r"$^2$"
            ...
            legend_title = (
                r"$\langle \Delta e^2(\Delta t) \rangle =$ (Bin Width)$^2$"
            )
        ax1.legend(loc="upper center", title=legend_title, ncol=3)
        if args.INFILE2 is not None:
        ...
        pdf.savefig()
        plt.close()
        # Plot the chosen MSDs as function of initial position
        if args.INFILE2 is None:
            fig, ax1 = plt.subplots(clear=True)
        else:
            height_ratios = (0.2, 1)
        ...
        if args.MDFILES is not None:
            ylabel = (
                r"True Var$[\Delta r(\Delta t)]$ / " + args.LUNIT + r"$^2$"
            )
        else:
            ylabel = (
                r"True $\langle \Delta r^2(\Delta t) \rangle$ / "
                + args.LUNIT
                + r"$^2$"
            )
        ax1.set(
