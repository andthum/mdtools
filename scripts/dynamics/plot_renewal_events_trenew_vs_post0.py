...
if __name__ == '__main__':
    ...
    for i in np.unique(bin_ix):
        ...
        trenew_mean[i] = np.mean(trenew[mask])
        trenew_std[i] = np.std(trenew[mask]) / np.sqrt(nevents)
    if not np.isnan(trenew_mean[0]):
        raise ValueError("The first element of trenew_mean is not NaN."
