...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            " inital position as scatter plot. Additionally, a"
            " bin-wise average is computed and plotted. The"
            " errorbars indicate the uncertainty of the average"
            " (standard deviation of the data divided by the"
    ...
    msd_std = np.full((len(bins), msd.shape[1]), np.nan)
    msd_tot_mean = np.full(len(bins), np.nan)
    msd_tot_std = np.full(len(bins), np.nan)
    for i in np.unique(bin_ix):
        mask = (bin_ix == i)
        nevents = np.count_nonzero(mask)
        msd_mean[i] = np.mean(msd[mask], axis=0)
        msd_std[i] = np.std(msd[mask], axis=0) / np.sqrt(nevents)
        msd_tot_mean[i] = np.mean(msd_tot[mask])
        msd_tot_std[i] = np.std(msd_tot[mask]) / np.sqrt(nevents)
    if not np.all(np.isnan(msd_mean[0])):
        raise ValueError("Not all first elements of msd_mean are NaN."
                         " This should not have happened")
    if not np.all(np.isnan(msd_std[0])):
        raise ValueError("Not all first elements of msd_std are NaN."
                         " This should not have happened")
    if not np.isnan(msd_tot_mean[0]):
        raise ValueError("The first element of msd_tot_mean is not NaN."
                         " This should not have happened")
    if not np.isnan(msd_tot_std[0]):
        raise ValueError("The first element of msd_tot_std is not NaN."
                         " This should not have happened")
