...
# Local imports
from lifetime_autocorr_serial import (
    autocorr_bound,
    autocorr_unbound,
    fit_kww,
    kww,
...
if __name__ == "__main__":  # noqa: C901
    ...
    additional_description = (
        " This script is parallelized.  The number of CPUs to use is specified"
        " (in decreasing precedence) by either one of the environment"
        " variables OMP_NUM_THREADS, SLURM_CPUS_PER_TASK,"
        " SLURM_JOB_CPUS_PER_NODE, SLURM_CPUS_ON_NODE or python intern by"
        " os.cpu_count().  Best performance is considered to be reached, when"
        " the number of used CPUs is 1-3 times the number of blocks for block"
        " averaging."
    )
    args = parse_user_input(add_description=additional_description)
    NCPUS = mdt.rti.get_num_CPUs()
    print("\n")
    print("Available CPUs: {}".format(NCPUS))
    ...
    NBLOCKS, blocksize = mdt.check.block_averaging(
        n_blocks=args.NBLOCKS, n_frames=N_FRAMES, check_CPUs=True
    )
    ...
    if BEGIN + (chunk + 1) * chunk_size < END:
        ...
        raise ValueError(
            "Read more frames than given with -e.  This should not have"
            " happened"
        )
    ...
    timer = datetime.now()
    pool = mdt.parallel.ProcessPool(nprocs=NCPUS)
    ...
    if len(acorr_bound) != NBLOCKS:
        raise ValueError(
            "The number of autocorrelation functions for bound"
            " reference-selection complexes is not equal to the number of"
            " blocks for block averaging.  This should not have happened"
        )
    if len(acorr_unbound_ref) != NBLOCKS:
        raise ValueError(
            "The number of autocorrelation functions for unbound reference"
            " compounds is not equal to the number of blocks for block"
            " averaging.  This should not have happened"
        )
    if len(acorr_unbound_sel) != NBLOCKS:
        raise ValueError(
            "The number of autocorrelation functions for unbound selection"
            " compounds is not equal to the number of blocks for block"
            " averaging.  This should not have happened"
        )
