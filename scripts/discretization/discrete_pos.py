...
-o
    Output filename for the discrete trajectory.  The discrete
    trajectory is written as binary :file:`dtrj.npy` file in a a
...
--bins-out
    Output filename for the bin edges (optional).  If provided, the
    (average) bin edges used for creating the discrete trajectory are
    written to a text file of the given filename.
-b
...
--cmp
    ...
    The compounds of the selection group whose center of mass positions
    should be discretized.  Compounds can be 'group' (the entire
    selection group), 'segments', 'residues', 'fragments', or 'atoms'.
    Refer to the MDAnalysis' user guide for an
    |explanation_of_these_terms|.  Compounds are made whole before
    calculating their centers of mass.  The centers of mass are wrapped
    back into the primary unit cell before discretizing their positions.
    Note that in any case, even if ``CMP`` is e.g. 'residues', only the
    atoms belonging to the selection group are taken into account, even
    if the compound might comprise additional atoms that are not
    contained in the selection group.  Default: ``'atoms'``.
-d  {'x', 'y', 'z'}

    Direction.  The spatial direction in which to bin the positions of
    the reference compounds.  Default: ``'z'``.
--bin-start
    Point (in Angstrom) on the chosen spatial direction to start
    binning.  Note that binning naturally starts at zero (origin of the
    simulation box).  If parsing a start value greater than zero, the
    first bin interval will be ``[0, START)``.  In this way you can
    determine the width of the first bin independently from the other
    bins.  Note that ``START`` must lie within the simulation box
    obtained from the first frame read and it must be smaller than
    ``STOP``.  Default: ``0``
--bin-end
    Point (in Angstrom) on the chosen spatial direction to stop binning.
    Note that binning naturally ends at ``lbox + tol`` (length of the
    simulation box in the given spatial direction plus a small tolerance
    to account for the right-open bin interval).  If parsing a value
    less than ``lbox``, the last bin interval will be ``[STOP,
    lbox+tol)``.  In this way you can determine the width of the last
    bin independently from the other bins.  Note that ``STOP`` must lie
    within the simulation box obtained from the first frame read and it
    must be greater than ``START``.  Default: ``lbox + tol``.
--bin-num
    Number of equidistant bins (not bin edges!) to use for discretizing
    the given spatial direction between ``START`` and ``STOP``.  Note
    that two additional bins, ``[0, START)`` and ``[STOP, lbox+tol)``,
    are created if ``START`` is not zero and ``STOP`` is not ``lbox``.
    Default: ``10``.
--bins
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Create a discrete position trajectory.  For more information,"
    ...
    if args.OUTFILE_BINS is not None:
        header = (
            ...
            "Time step of discrete trajectory:     {:<.3f} ps\n".format(
                len(bins), len(bins) - 1, args.DIRECTION, lbox_av, time_step
            )
        )
