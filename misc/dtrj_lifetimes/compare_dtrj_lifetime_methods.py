...
--sf
    Name of the file containing the survival functions for each state as
    generated by :mod:`scripts.discretization.state_lifetime_discrete`.
--param
...
--int-thresh
    Only calculate the lifetime by directly integrating the survival
    function if the survival function decays below the given threshold.
    Default: ``0.01``.
--end-fit
...
See Also
--------
:mod:`misc.dtrj_lifetimes.generate_dtrj.py` :
...
if __name__ == "__main__":  # noqa: C901
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Name of the file containing the survival functions for each"
            " state."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Only calculate the lifetime by directly integrating the survival"
            " function if the survival function decays below the given"
    ...
    print("\n")
    print("Calculating lifetimes from the survival function (Methods 3-5)...")
    timer = datetime.now()

    surv_funcs = np.loadtxt(args.INFILE_SF)
    states = surv_funcs[0, 1:]  # State indices.
    times = surv_funcs[1:, 0]  # Lag times in trajectory steps.
    surv_funcs = surv_funcs[1:, 1:]  # Survival function functions.
    if np.any(surv_funcs < 0) or np.any(surv_funcs > 1):
        raise ValueError(
            "Some values of the survival function lie outside the interval"
            " [0, 1]"
        )
    ...
    # Method 3: Set the lifetime to the lag time at which the survival
    # function crosses 1/e.
    thresh = 1 / np.e
    ix_thresh = np.nanargmax(surv_funcs <= thresh, axis=0)
    lifetimes_e = np.full(n_states, np.nan, dtype=np.float64)
    for i, sf in enumerate(surv_funcs.T):
        if sf[ix_thresh[i]] > thresh:
            # The survival function never falls below the threshold.
            lifetimes_e[i] = np.nan
        elif ix_thresh[i] < 1:
            # The survival function immediately falls below the
            # threshold.
            lifetimes_e[i] = 0
        elif sf[ix_thresh[i] - 1] < thresh:
            raise ValueError(
                "The threshold ({}) does not lie within the survival function"
            ...
            slope = sf[ix_thresh[i]] - sf[ix_thresh[i] - 1]
            slope /= times[ix_thresh[i]] - times[ix_thresh[i] - 1]
            intercept = sf[ix_thresh[i] - 1] - slope * times[ix_thresh[i] - 1]
    ...
    # Method 4: Calculate the lifetime as the integral of the survival
    # function.
    ...
    for i, sf in enumerate(surv_funcs.T):
        valid = ~np.isnan(sf)
        lifetimes_int_mom1[i] = np.trapz(y=sf[valid], x=times[valid])
        lifetimes_int_mom2[i] = np.trapz(
            y=sf[valid] * times[valid], x=times[valid]
        )
        lifetimes_int_mom3[i] = np.trapz(
            y=sf[valid] * (times[valid] ** 2), x=times[valid]
        )
        lifetimes_int_mom3[i] /= 2
    invalid = np.all(surv_funcs > args.INT_THRESH, axis=0)
    lifetimes_int_mom1[invalid] = np.nan
    lifetimes_int_mom2[invalid] = np.nan
    lifetimes_int_mom3[invalid] = np.nan
    del valid, invalid

    # Method 5: Fit the survival function with a stretched exponential
    ...
    for i, sf in enumerate(surv_funcs.T):
        stop_fit = np.nanargmax(sf < args.STOPFIT)
        if stop_fit == 0 and sf[stop_fit] >= args.STOPFIT:
            stop_fit = len(sf)
        elif stop_fit < 2:
            stop_fit = 2
        ...
        popt[i], perr[i] = mdt.func.fit_kww(
            xdata=times_fit, ydata=sf_fit, p0=init_guess[i], method="trf"
        )
        if np.any(np.isnan(popt[i])):
            ...
            ss_res = np.nansum((sf_fit - fit) ** 2)
            # Mean squared error / mean squared residuals.
            fit_mse[i] = ss_res / len(fit)
            # Total sum of squares
            ss_tot = np.nansum((sf_fit - np.nanmean(sf_fit)) ** 2)
    ...
    header = (
        ...
        + "1-2) or from the corresponding survival function function (Method\n"
        + "3-4).\n"
        + "\n"
        + "\n"
        + "Discrete trajectory: {:s}\n".format(args.INFILE_DTRJ)
        + "Survival function:   {:s}\n".format(args.INFILE_SF)
        + "\n"
        + "\n"
        ...
        + "   survival function function S(t) crosses 1/e.  If this never\n"
        ...
        + "   as the integral of the survival function function S(t) times\n"
        + "   t^(n-1):\n"
        + "     <tau_int^n> = 1/(n-1)! int_0^inf t^(n-1) S(t) dt\n"
        + "   If S(t) does not decay below the given threshold of\n"
        + "   {:.4f}, <tau_int^n> is set to NaN.\n".format(args.INT_THRESH)
        + "\n"
        + "5) The survival function function S(t) is fitted by a stretched\n"
        ...
        + "   confined to the interval [0, 1].  The survival function is\n"
    ...
    with PdfPages(outfile) as pdf:
        ...
        # Plot survival functions and fits for each state.
        fig, ax = plt.subplots(clear=True)
        ax.set_prop_cycle(color=colors)
        for i, sf in enumerate(surv_funcs.T):
            times_fit = times[fit_start[i] : fit_stop[i]]
            fit = mdt.func.kww(times_fit, *popt[i])
            lines = ax.plot(times, sf, label=r"$%d$" % states[i], linewidth=1)
            ax.plot(
        ...
        for i, sf in enumerate(surv_funcs.T):
            times_fit = times[fit_start[i] : fit_stop[i]]
            fit = mdt.func.kww(times_fit, *popt[i])
            res = sf[fit_start[i] : fit_stop[i]] - fit
