...
if __name__ == "__main__":  # noqa: C901
    ...
    lifetimes_cnt, states = mdt.dtrj.lifetimes_per_state(
        dtrj,
    ...
    print("\n")
    print("Calculating the remain probability...")
    ...
    timer = datetime.now()
    remain_props = mdt.dtrj.remain_prob_discrete(
        dtrj1=dtrj,
        dtrj2=dtrj,
    ...
    del dtrj
    times = np.arange(len(remain_props))
    print("Elapsed time:         {}".format(datetime.now() - timer))
    print("Current memory usage: {:.2f} MiB".format(mdt.rti.mem_usage(proc)))
    ...
    xlim = (states[0] - 0.5, states[-1] + 0.5)
    alpha = 0.75
    ...
    with PdfPages(outfile) as pdf:
        ...
        ax.set(
            xlabel=xlabel, ylabel=r"Mean Squared Error / Frames$^2$", xlim=xlim
        )
        ax.xaxis.set_major_locator(MaxNLocator(integer=True))
        pdf.savefig()
